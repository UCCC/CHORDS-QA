...
```{r tab3_res, echo = F, include=T, error=T}
tab3 <- run_query( 
...
```{r tab4_res, echo = F, include=T, error=T}
tab4 <- run_query( 
...
```{r fig1_res, fig.height=3.5, fig.width=7, echo = F, include=T, error=T, dpi=500}
fig1 <- run_query(
  paste0("select test_type, 
...
invisible(lapply(fig1Figures, print))
...
```{r tab5_res, echo = F, include=T, error=T}
tab5 <- run_query(
...
```{r tab6_res, echo = F, include=T, error=T}
tab6 <- run_query(
...
```{r tab7_res, echo = F, include=T, error=T}
tab7<- run_query( 
...
```{r tab8_res, echo = F, include=T, error=T}
tab8 <- run_query( 
...
```{r tab8_dis, echo = F, include=T, error=T}
...
```{r tab9_disp, echo = F, include=T, error=T}
...
```{r tab10_res, echo = F, include=T, error=T}
...
```{r tab11_res, echo = F, include=T, error=T}
...
```{r tab12_disp, echo = F, include=T, error=T}
...
```{r tab13_disp, echo = F, include=T, error=T}
...
```{r tab15_res, echo = F, include=T, error=T}
tab15 <- run_query(
...
```
```{r tab15_disp, echo = F, include=T, error=T}
...
```{r tab16_res, echo = F, include=T, error=T}
tab16 <- run_query(
    paste0("select year(contact_date) as year,
    count(*) as num_rec,
...
```{r tab16_disp, echo = F, include=T, error=T}
...
```{r tab17_disp, echo = F, include=T, error=T}
...
## Table 18: PHQ-2 Response Detail

```{r tab18_res, echo = F, include=T, error=T}
tab18 <- run_query(
...
tab18$pct_high_response <- round(100*tab18$n_high_scores/tab18$nresponse,1)
```

```{r tab18_disp, echo = F, include=T, error=T}
kable(tab18, col.names = c("Question ID", "# of Responses", "# Null Responses", "# of Duplicates",  "# of Encounters", "# of Patients", "Minimum Response Value", "Maximum Response Value", "Average Response Value", "# High Responses", "% High Responses"), align = 'c', format.args = list(big.mark = ","))
```

## Table 19: PRO_Responses: Surveys and Questions included
...
```{r tab19_res,echo = F, include=T, error=T }
tab19 <- run_query(
...
tab19_order <- tab19[order(tab19$pro_id, tab19$question_id),]
	  
```

```{r tab19_disp, echo = F, include=T, error=T}
kable(tab19_order, col.names = c("PRO_ID", "QUESTION_ID", "Number of Records", "Minimum Value", "Maximum Value", "Response Number Null", "Response Text Null", "Different values for Response Number and Text"), row.names = F, align = 'c', format.args = list(big.mark = ","))
```

```{r tab20a,echo = F, include=T, error=T }

tab20_years_f <- function(){
...
tab20_years <- tab20_years_f()

```

## Table 20: PHQ-2 Number of Encounters and Percent of Encounters Screened by Year - All Encounter Types (`r tab20_years[1]` - Present)

```{r tab20_res,echo = F, include=T, error=T }
#All encounters
tab20_enc <- run_query( 
        paste0("
        with cte_years as (
          SELECT '",tab20_years[1] ,"' year
          union
          SELECT '",tab20_years[2] ,"'
          union
          SELECT '",tab20_years[3] ,"'
          union
          SELECT '",tab20_years[4] ,"'
          union
          SELECT '",tab20_years[5] ,"'
        )
        ...
        left join(
          select year(adate) as year,
	        count(distinct enc_id) as nenc
	        from ", encounters,"
	        where year(adate) in (", paste0("'", tab20_years[1],"', ", "'",tab20_years[2], "', ", "'", tab20_years[3], "', ", "'", tab20_years[4], "', ", "'", tab20_years[5], "'"), ")
          group by year(adate)
...
tab20_screen <- run_query( 
        ...
        left join(
          select question_id,
	        ...
	        where pro_id='CHORDS-1'
	          and question_id='3'
	          and year(response_date) in (", paste0("'", tab20_years[1],"', ", "'",tab20_years[2], "', ", "'", tab20_years[3], "', ", "'", tab20_years[4], "', ", "'", tab20_years[5], "'"), ")
...
tab20_total <- merge(tab20_enc, tab20_screen, by="year")
...
#Year1
tab20_total_wide$screen_1 <- with(tab20_total_wide, if (paste0("nscreen.", tab20_years[1]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nscreen.", tab20_years[1])] else 0 )
tab20_total_wide$enc_1 <- with(tab20_total_wide, if (paste0("nenc.", tab20_years[1]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nenc.", tab20_years[1])]  else 0 )
...
#Year2
tab20_total_wide$screen_2 <- with(tab20_total_wide, if (paste0("nscreen.", tab20_years[2]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nscreen.", tab20_years[2])] else 0 )
tab20_total_wide$enc_2 <- with(tab20_total_wide, if (paste0("nenc.", tab20_years[2]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nenc.", tab20_years[2])]  else 0 )
...
#Year3
tab20_total_wide$screen_3 <- with(tab20_total_wide, if (paste0("nscreen.", tab20_years[3]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nscreen.", tab20_years[3])] else 0 )
tab20_total_wide$enc_3 <- with(tab20_total_wide, if (paste0("nenc.", tab20_years[3]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nenc.", tab20_years[3])]  else 0 )
...
#Year4
tab20_total_wide$screen_4 <- with(tab20_total_wide, if (paste0("nscreen.", tab20_years[4]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nscreen.", tab20_years[4])] else 0 )
tab20_total_wide$enc_4 <- with(tab20_total_wide, if (paste0("nenc.", tab20_years[4]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nenc.", tab20_years[4])]  else 0 )
...
#Year5
tab20_total_wide$screen_5 <- with(tab20_total_wide, if (paste0("nscreen.", tab20_years[5]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nscreen.", tab20_years[5])] else 0 )
tab20_total_wide$enc_5 <- with(tab20_total_wide, if (paste0("nenc.", tab20_years[5]) %in% colnames(tab20_total_wide)) tab20_total_wide[paste0("nenc.", tab20_years[5])]  else 0 )
...
```{r tab20_disp, echo = F, include=T, error=T}
kable(tab20_final, col.names = c("Question ID", paste0("# (%) Screened ", tab20_years[1]), paste0("# (%) Screened ", tab20_years[2]), paste0("# (%) Screened ", tab20_years[3]),  paste0("# (%) Screened ", tab20_years[4]), paste0("# (%) Screened ", tab20_years[5])), row.names = F, align = 'c', format.args = list(big.mark = ","))
```
