...
```{r tab4_res, echo = F, include=T, error=F}
tab4 <- run_query( 
...
```{r fig1_res, fig.height=3.5, fig.width=7, echo = F, include=T, error=F, dpi=500}
fig1 <- run_query(
  paste0("select test_type, 
...
```{r tab14_res, echo = F, include=T, error=F}
##debug code
...
tab18 <- run_query("select * from pro_surveys")
```
...
tab19 <- run_query("select * from pro_questions")
```
