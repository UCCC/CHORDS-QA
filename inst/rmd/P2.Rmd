...
if (file.exists(tableReplaces)){
  ...
  prescribing <- ifelse("prescribing" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("prescribing"), tolower(df$ORG_NAME))], "prescribing")
  procedures <- ifelse("procedures" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("procedures"), tolower(df$ORG_NAME))], "procedures")
  social_history <- ifelse("social_history" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("social_history"), tolower(df$ORG_NAME))], "social_history")
  provider_specialty <- ifelse("provider_specialty" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("provider_specialty"), tolower(df$ORG_NAME))], "provider_specialty")
...
tab1_ps <- odbc::dbGetQuery(con, paste0("select count(*) as nrows
...
tab2_rx_null <- odbc::dbGetQuery(con, 
  paste0("SELECT SUM(CASE WHEN GENERIC_MED_NAME IS NULL THEN 1 ELSE 0 END) as GENERIC_MED_NAME,
...
tab2_sh_null <- odbc::dbGetQuery(con, 
  paste0("SELECT SUM(CASE WHEN TOBACCO_USER IS NULL THEN 1 ELSE 0 END) as TOBACCO_USER,
...
tab2_sh_uk <- odbc::dbGetQuery(con, 
  paste0("SELECT SUM(CASE WHEN TOBACCO_USER = 'U' THEN 1 ELSE 0 END) as TOBACCO_USER,
