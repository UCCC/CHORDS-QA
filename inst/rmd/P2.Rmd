...
```{r fig1_res, fig.height=4, fig.width=7, echo = F, include=T, error=T}
...
tab18 <- odbc::dbGetQuery(con, "select * from pro_surveys")
```
...
tab19 <- odbc::dbGetQuery(con, "select * from pro_questions")
```

```{r tab19_disp, echo = F, include=T, error=T}
kable(tab19)
```
## Table 20: PRO_Responses: Surveys and Questions included
...
tab20_order <- tab20[order(tab20$pro_id, tab20$question_id),]
	  
```

```{r tab20_disp, echo = F, include=T, error=T}
kable(tab20_order, col.names = c("PRO_ID", "QUESTION_ID", "Number of Records", "Minimum Value", "Maximum Value", "Response Number Null", "Response Text Null", "Different values for Response Number and Text"), row.names = F, align = 'c', format.args = list(big.mark = ","))
```
