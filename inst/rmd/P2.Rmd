...
tab1_lab <- odbc::dbGetQuery(con, "select count(*) as nrows, 
  ...
  'RESULT_DT' as fieldname,
	cast(min(result_dt) as varchar) as mindt,
	cast(max(result_dt) as varchar) as maxdt
	from lab_results")
tab1_lab2 <- data.frame(nrows = tab1_lab$nrows, 
                       npats = tab1_lab$npats, 
                       nencts = NA, 
                       fieldname = tab1_lab$fieldname, 
                       mindt1 = as.Date(tab1_lab$mindt, format = "%B %d %Y"), 
                       mindt = as.character(tab1_lab2$mindt1),
                       maxdt1 = as.Date(tab1_lab$maxdt, format = "%B %d %Y"),
                       maxdt = as.character(tab1_lab2$maxdt1))

tab1_lab <- tab1_lab2[-c(5,7)]
...
tab2_sh_null <- odbc::dbGetQuery(con, 
  "SELECT SUM(CASE WHEN TOBACCO_USER='' THEN 1 ELSE 0 END) as TOBACCO_USER,
...
tab2_px_uk <- odbc::dbGetQuery(con, 
  "SELECT SUM(CASE WHEN PROVIDER = 'UNK' THEN 1 ELSE 0 END) as PROVIDER
      FROM procedures")
...
tab2_rx_uk <- odbc::dbGetQuery(con, 
  "SELECT SUM(CASE WHEN RXMD = 'UNK' THEN 1 ELSE 0 END) as RXMD
      FROM prescribing")
...
tab2_sh_uk <- odbc::dbGetQuery(con, 
  "SELECT SUM(CASE WHEN TOBACCO_USER = 'U' OR TOBACCO_USER = 'X' THEN 1 ELSE 0 END) as TOBACCO_USER,
...
kable(tab4, col.names = c("Test Type", "Number of Recorded Lab Tests", "Percentage of All Recorded Tests", "Number of Unique Patients Tested", "Percentage of All Patients", "Mean Result Value", "Minimum Result Value", "Maximum Result Value"), digits = 2, format.args = list(big.mark = ",")) 
```
  ...
  "select test_type, 
    ...
    count(*) as nrows
      ...
      where test_type in ('HGBA1C', 'PREGNANCY', 'HDL', 'LDL', 'TRIGLYCERIDE', 'GLUCOSE', 'CREATININE', 'HEP_C_RNA', 'HEP_C_ANTIBODY', 'HEP_C_GENOTYPE', 'IGE', 'EOSINOPHIL')
	    group by test_type, year(result_dt)") 
...
fig1_wide <- spread(fig1, key = test_type, value = nrows)



lapply(2:ncol(fig1_wide), function(x) 
  ggplot(data = fig1_wide, aes(x = year, y = fig1_wide[[x]])) +
         geom_line() +
    ...
    theme(panel.grid.major = element_blank(), 
        ...
        axis.title = element_text(size = 16, face = "bold", family="TT Arial"),
        axis.text = element_text(size = 14, face = "bold", family="TT Arial"),
        plot.title = element_text(hjust = 0.5, size = 20, family="TT Arial"))
    ) 
