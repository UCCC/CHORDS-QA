...
FROM ", benefit))
tab2_benefit_null2 <- data.frame(table = rep("BENEFITS", ncol(tab2_benefit_null)), var = names(tab2_benefit_null), records_null = t(tab2_benefit_null)[,1])
                             ...
                             ifelse(table == "CENSUS_LOCATION", (records_null / tab1_cen$nrows * 100), 
                                     ifelse(table == "DIAGNOSES", (records_null / tab1_dx$nrows * 100), 
                                             ifelse(table == "VITAL_SIGNS", (records_null / tab1_vs$nrows * 100),
                                                    ifelse(table == "BENEFITS", (records_null / tab1_ben$nrows * 100),
                                                           NA))))))
  )
...
```{r telehealth_tab7.14, echo = F, include=T, error=T}
...
tab7.14a <- flextable(format(data = tab7.14a, big.mark=",")) %>% 
...
tab7.14b <- flextable(format(data = tab7.14b, big.mark=",")) %>% 
  set_table_properties(layout = "autofit") %>% 
...
tab7.14c <- flextable(data = tab7.14c) %>% 
  set_table_properties(layout = "autofit") %>% 
...
if (nrow(tab14_09) != 0){
  tab14_09 <- tab14_09 %>% 
    ...
    left_join(., icdLU, by = "dxCdTp") %>% 
  select(dx, dxDesc, nrows_all, npats)
...
```{r Count of vital signs by year and month, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
