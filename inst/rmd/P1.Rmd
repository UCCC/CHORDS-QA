...
```{tab17_disp, results = "asis", eval=FALSE}

qwraps2::summary_table(tab17_final, tab17_display, digits = 2)

```

## Table 18: CENSUS_LOCATIOn

```{r tab18_out, results="asis", eval=FALSE}

tab18a <- odbc::dbGetQuery(con, 
                          "SELECT COUNTY_GEOCODE,
                          COUNT(*) as num_county
                          FROM CENSUS_LOCATION
                          GROUP BY COUNTY_GEOCODE")

tab18b <- odbc::dbGetQuery(con, 
                          "SELECT ADDRESS_FLAG,
                          COUNT(*) as num_add_flag
                          FROM CENSUS_LOCATION
                          GROUP BY ADDRESS_FLAG")

tab18_tot <- odbc::dbGetQuery(con, 
                          "COUNT(*) n_census
                           FROM CENSUS_LOCATION")

tab18_all <- dplyr::bind_cols(tab18a, tab18b) %>% 
  mutate(
  perc_county = paste0(round((num_county / n_census) * 100, "%"), 1),
  perc_add_flag = paste0(round((num_add_flag / n_census) * 100, "%"), 1)
  )

tab18_all <- tab18_all[, c(1, 2, 5, 3, 4, 6)]

knitr::kable(tab18_all, col.names = c("County Geocode", "Frequency County Geocode", "Percent County Geocode", "Address Flag", "Frequency Address Flag", "Percent Address Flag"), format.args = list(big.mark = ","))
...
tab19 <- odbc::dbGetQuery(con, 
                          "SELECT COUNT DISTINCT(PERSON_ID) as npats,
