  ...
  tryCatch(
    {
      ...
      #if (length(extra_arguments) > 0){
      #  result <-RODBC::sqlQuery(channel = db_conn, query = query_text, extra_arguments)
      #}else{
...
```{r vital signs summary, echo = T, include=T}
...
vitalVarSumm <- function(var,label){
  vitalCon <- DBI::dbConnect(odbc::odbc(),.connection_string = connectionString)
  vitalResult <- odbc::dbGetQuery(vitalCon,
                 paste0('WITH ranks
