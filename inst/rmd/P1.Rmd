...
## Table 10.1. Count of Encounter Subtypes by Encounter Type 
```{r tab10.1, include=F}



tab_enc_subtype <- DBI::dbGetQuery(con, gsub("%ENCOUNTERS%", encounters, "
SET NOCOUNT ON;
declare @ENCSUBTYPE TABLE (ENCTYPES varchar(2),  ENCTYPEDESC varchar(50));
insert into @ENCSUBTYPE values 
       ('AV', 'AV: Ambulatory Visits')
       ,('IP', 'IP: Acute Inpatient Hospital Stays')
       ,('ED', 'ED: Emergency Department')
       ,('TE', 'TE: Telephone Encounters')
       ,('EM', 'EM: E-mail Encounters')
       ,('IS', 'IS: Non-Acute Institutional Stays')
       ,('OE', 'OE: Other Encounters (not overnight)')
       ,('LO', 'LO: Lab Only Encounters')
       ,('RO', 'RO: Radiology Only Encounters');
 
select ENCTYPEDESC as enc_types,
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'AI' in (encounter_subtype)) as 'AI',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'DI' in (encounter_subtype)) as 'DI',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'HH' in (encounter_subtype)) as 'HH',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'HA' in (encounter_subtype)) as 'HA',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'HS' in (encounter_subtype)) as 'HS',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'NH' in (encounter_subtype)) as 'NH',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'OB' in (encounter_subtype)) as 'OB',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'OC' in (encounter_subtype)) as 'OC',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'RH' in (encounter_subtype)) as 'RH',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'SD' in (encounter_subtype)) as 'SD',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'SN' in (encounter_subtype)) as 'SN',
       (select count(*)
        from %ENCOUNTERS%
        where  ENCTYPES in (enctype) and
              'UC' in (encounter_subtype)) as 'UC'
              
  from @ENCSUBTYPE;"))                
              

#Row totals
rownames(tab_enc_subtype) <- tab_enc_subtype$enc_types
tab_enc_subtype$enc_types <- NULL
rowsum <-colSums(tab_enc_subtype)
tab_enc_subtype <-rbind(tab_enc_subtype, rowsum)


#Column totals
Total <-rowSums(tab_enc_subtype)
tab_enc_subtype <- cbind(tab_enc_subtype,colsum)


#Final display
tab_enc_subtype$AI = paste0(format(tab_enc_subtype$AI,big.mark = ",") ,' (',round(tab_enc_subtype$AI/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$DI = paste0(format(tab_enc_subtype$DI,big.mark = ",") ,' (',round(tab_enc_subtype$DI/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$HH = paste0(format(tab_enc_subtype$HH,big.mark = ",") ,' (',round(tab_enc_subtype$HH/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$HA = paste0(format(tab_enc_subtype$HA,big.mark = ",") ,' (',round(tab_enc_subtype$HA/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$HS = paste0(format(tab_enc_subtype$HS,big.mark = ",") ,' (',round(tab_enc_subtype$HS/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$NH = paste0(format(tab_enc_subtype$NH,big.mark = ",") ,' (',round(tab_enc_subtype$NH/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$OB = paste0(format(tab_enc_subtype$OB,big.mark = ",") ,' (',round(tab_enc_subtype$OB/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$OC = paste0(format(tab_enc_subtype$OC,big.mark = ",") ,' (',round(tab_enc_subtype$OC/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$RH = paste0(format(tab_enc_subtype$RH,big.mark = ",") ,' (',round(tab_enc_subtype$RH/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$SD = paste0(format(tab_enc_subtype$SD,big.mark = ",") ,' (',round(tab_enc_subtype$SD/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$SN = paste0(format(tab_enc_subtype$SN,big.mark = ",") ,' (',round(tab_enc_subtype$SN/tab_enc_subtype$colsum*100, digits=1),'%)')
tab_enc_subtype$UC = paste0(format(tab_enc_subtype$UC,big.mark = ",") ,' (',round(tab_enc_subtype$UC/tab_enc_subtype$colsum*100, digits=1),'%)')


encsubLabels <-c('AI: Acute Inpatient Stay', 'DI: Dialysis','HH: Home Health','HA: Hospital Ambulatory', 'HS: Hospice', 'NH: Nursing Home','OB: Observation Bed','OC: Outpatient Clinic Visit', 'RH: Rehab','SD: Same Day Surgery', 'SN: SNF','UC: Urgent Care', 'Total' )

#Renaming for final table
colnames (tab_enc_subtype) <- encsubLabels
rownames(tab_enc_subtype) [10] <- "Total"

tab_enc_subtype <- tab_enc_subtype %>%
  rownames_to_column((var="Encounter Type"))

```

```{r tab10.1_disp, echo=FALSE}
knitr::kable(tab_enc_subtype,
             row.names=FALSE, align = 'c',
             format.args = list(big.mark = ","))
```
