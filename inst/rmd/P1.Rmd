...
```{r Demographic table, include=F, error=T}

nDemogRows <- odbc::dbGetQuery(con, paste0("select  count(*) as nDemogRows FROM ",   demographics)) 
demogSummary <- odbc::dbGetQuery(con, 
...
```{r demographic counts, include=F, error=T}
demogCounts <- odbc::dbGetQuery(con, 
...
```{r missing race/ethnicity over time, include=FALSE, error=T}
...
```{r missing race/ethnicity by age, include=FALSE, error=T}
...
```{r encounter table, include=FALSE, error=T}
rm(diag)
gc()
...
```{r encounter types by year and month, include=F, error=T}
...
```{r add labels to encEncType_YM , include=F, error=T}
