...
if (nchar(params$DBUser) == 0)
{
  ...
  sqldb <- odbcDriverConnect(paste('driver={SQL Server};server=',params$DBServerName,';database=',params$DBName,';connection timeout=5000', sep=""), readOnlyOptimize = TRUE)
} else {
	con <- DBI::dbConnect(odbc::odbc(),
                      Driver = "SQL Server",
                      Server = params$DBServerName,
	...
	sqldb <- odbcDriverConnect(paste('driver={SQL Server};uid=',params$DBUser,';pwd=',params$DBPassword,';server=',params$DBServerName,';database=',params$DBName,';connection timeout=5000', sep=""), readOnlyOptimize = TRUE)
}
...
```{r supporting data and functions, include = FALSE, error=T}
## helper functions
...
dxByMonEncTp <- odbc::dbGetQuery(con,
...
vitalsByMonYr <- odbc::dbGetQuery(con,
