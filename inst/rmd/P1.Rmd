...
ICD10CM <- chordsTables::ICD10CM
ICD9CM <- chordsTables::ICD9CM
...
```{r missing race/ethnicity over time, echo = F, include=T, error=T, eval=T}
...
```{r tab7.11_fig, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r telehealth_tab7.14a, echo = F, include=T, error=T}
...
```{r telehealth_tab7.14b, echo = F, include=T, error=T}
...
```{r telehealth_tab7.14c, echo = F, include=T, error=T}
...
```{r tab8, echo = F, include=T, error=T, eval=T}
...
```{r fig2, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r tab10, echo = F, include=T, error=T, eval=T}
...
```{r add labels to encEncType_YM , echo = F, include=T, error=T, eval=T}
...
fig3_timeTest <-  timeTest(time=as.Date(encTypeBymonYr$adate_YM), 
                           Freq=encTypeBymonYr$Freq, 
                           outcome=encTypeBymonYr$encType )
fig3_timeTest <- merge(fig3_timeTest,encLabels, by.x='outcome', by.y='code', all.x=TRUE) 
fig3_timeTest <- within(fig3_timeTest,{decode <- ifelse(!is.na(decode),decode, ifelse(is.na(encType), 'Missing', 'Invalid'))})
...
```{r fig3a, echo = F, include=T, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r fig3b, echo = F, include=T, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r lastLocation, echo = F, include=T, echo = F, include=T, error=T, eval=T}
...
```{r tab13_years, echo = F, include=T, error=T}
...
## Table 13. Summary  of the Ten Most Common Diagnosis Codes, by All or Ambulatory Encounter Type ( `r paste0(tab_2_Last_Recent_Years_f[0], " & ",tab_2_Last_Recent_Years_f[1])`)
...
tab13_dxall <- run_query( 
  paste0("select dx,
    ...
    count(distinct person_id) as npats
	  from ", diagnoses, "
	  where YEAR(ADATE) IN (", tab_2_Last_Recent_Years_f[0],", ",tab_2_Last_Recent_Years_f[2], " )
...
tab13_dxav <- run_query( 
  paste0("select dx as dx_av,
    ...
    count(distinct person_id) as npats_av
	  from ", diagnoses, "
      where ENCTYPE = 'AV'
      YEAR(ADATE) IN (", tab_2_Last_Recent_Years_f[0],", ",tab_2_Last_Recent_Years_f[2], " )
	    group by dx, dx_codetype")) %>% 
...
fig4_timeTest <- within(fig4_timeTest,{decode <- ifelse(!is.na(decode),decode, ifelse(is.na(encType), 'Missing', 'Invalid'))})

```

## Figure 4a. Count of Diagnoses by Year and Month in Ambulatory Encounters 

```{r Count of diagnoses by year and month in ambulatory encounters, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r Count of diagnoses by year and month other encounter types, echo = F, include=T, error=T, dpi=500, fig.height=5, fig.width=8.5}
...
```{r Count of vital signs by year and month, echo=F, error=T, dpi=500, include=T, fig.show="hold", fig.height=3.25, fig.width=4.75, fig.align='center', out.width="50%"}
