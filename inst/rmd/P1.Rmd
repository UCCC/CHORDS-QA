...
tab17_ben_null <- odbc::dbGetQuery(con, 
  "SELECT COUNT DISTINCT(BENEFIT_ID) as total,
...
tab17_final <- tab17_ben_null %>% 
  mutate(
    perc_null_load = LOAD_DATE / total,
    perc_null_refresh = REFRESH_DATE / total,
    perc_null_start = START_DATE / total,
    perc_null_end = END_DATE / total
  )

tab17_display  <- list(
  "LOAD_DATE" =
    list("Minimum" = ~ MIN_LOAD_DATE,
         "Maximum" = ~ MAX_LOAD_DATE,
         "Percent Null" = ~ paste0(perc_null_load, "%")),
  "REFRESH_DATE" =
    list("Minimum" = ~ MIN_REFRESH_DATE,
         "Maximum" = ~ MAX_REFRESH_DATE,
         "Percent Null" = ~ paste0(perc_null_refresh, "%")),
  "END_DATE" =
    list("Minimum" = ~ MIN_START_DATE,
         "Maximum" = ~ MAX_START_DATE,
         "Percent Null" = ~ paste0(perc_null_start, "%")),
  "END_DATE" =
    list("Minimum" = ~ MIN_END_DATE,
         "Maximum" = ~ MAX_END_DATE,
...
qwraps2::summary_table(tab17_final, tab17_display)
