...
if (params$QAAlert == TRUE){
  ...
  if (!(length(code_conform_table_counts_table)==0)){
    code_conform_table_counts_table$message <- paste0(code_conform_table_counts_table$TableName, ".", code_conform_table_counts_table$CodeType, sep="")
...
```{r echo=F}
knitr::kable(nullableValues, row.names = FALSE, col.names = c("Table Name", "Column Name", "Expected Field Allows Null Value", "Found Field Allows Null Value"))
```
...
```{r echo=F}
knitr::kable(GR1Person,col.names = c('Table','Number of Encounters','Encounters Assinged to More Than One Person'), row.names = FALSE)
```
...
## Data Check 2.02: More than 10% of records fall into the lowest or highest categories of age, height, weight, diastolic blood pressure, systolic blood pressure, or dispensed days supply
```{r echo=F}
outOfRangeMsg <- NULL
...
check3.03Msg <- if(nrow(subset(result3.03, pctBad>=10.0))>0 & params$QAAlert==F){
        knitr::kable(subset(result3.03, pctBad>=10.0)[,c('dataTable','variable','N_rows','N_bad', 'pctBad')], 
                    col.names = c('Table','Variable','N Rows','N with bad values','%'))
} else if(nrow(subset(fdatesdone, pctFuture>=5.0))==0 & params$QAAlert==F) {
                   "No check exceeds the 10% limit on bad values."
} else if ( params$QAAlert==T) {
