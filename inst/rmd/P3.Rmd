...
schema_validation <- sqlQuery(connection103, schema_validation_query)
...
config_mismatch <- schema_validation[schema_validation$Result == "CONFIG-MISMATCH", ]

invalid_specs <- select(config_mismatch, c("TableName", "ColumnName", "ExpectedNumberPrecision", "FoundNumberPrecision", "ExpectedNumberScale", "FoundNumberScale", "ExpectedCharLength", "FoundCharLength", "ExpectedDatePrecision", "FoundDatePrecision"))
...
knitr::kable(orphan_person_ids, row.names = FALSE)
```
...
knitr::kable(orphan_enc_ids, row.names = FALSE)
```
...
knitr::kable(code_conform_table_counts, row.names = FALSE)
```

```{r 1.13 Top 50 Invalid COdes, echo=F }
knitr::kable(top_50_invalid, row.names = FALSE ,caption = "Top 50 Invalid Codes")
```
...
knitr::kable(ipedei_no_principal_diag, row.names = FALSE)
```
...
knitr::kable(data_result,row.names = FALSE)
