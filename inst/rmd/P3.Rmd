...
for(i in 1:length(nonMissingDBTables)){
  table_N_lst[[i]] <- sqlQuery(sqldb, paste("select distinct '_aaa_' as _aaa_, count(*) as tabN from ",nonMissingDBTables[i], sep=' '))#[,'tabN',drop=T]
 # print(dbTables[i])
...
```{r echo = F, include=T, error=T}
# DC 2.01
tabDates <- sqlQuery(sqldb, 'select distinct a.name as tabname, b.name as colname from sys.tables as a inner join sys.columns as b on a.object_id = b.object_id order by a.name') %>% 
  subset(grepl('date',colname, ignore.case = T))
  
modDates <- sqlQuery(sqldb, 'select name, modify_date from sys.tables')
...
test <- sqlQuery(sqldb, futureDateCode[1, "sqlcode"])

fdatelst <- list()
for (i in 1:nrow(futureDateCode)){
  fdatelst[[i]] <- sqlQuery(sqldb, futureDateCode[i, "sqlcode"])
}
...
patAgeRange <- sqlQuery(sqldb, paste0("
...
htOutOfRange <- sqlQuery(sqldb, paste0("
...
wtOutOfRange <- sqlQuery(sqldb, paste0("
...
dbpOutOfRange <- sqlQuery(sqldb, paste0("
...
from ", vital_signs, " 
 where diastolic is not null
                             "))
sbpOutOfRange <- sqlQuery(sqldb, paste0("
...
daysSupplyOutOfRange <- sqlQuery(sqldb, paste0("
...
encTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
encTabDeath2.03 <- sqlQuery(sqldb,paste0("
...
procTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
procTabDeath2.03 <- sqlQuery(sqldb,paste0("
 select sum(deathPreProc) as deathPreProc
  from (
                       ...
                       ") )
vitalsTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
vitalsTabDeath2.03 <- sqlQuery(sqldb,paste0("
 select sum(deathPreMeasure) as deathPreMeasure
  from (
                       ...
                       ") )
pharmTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
pharmTabDeath2.03 <- sqlQuery(sqldb,paste0("
...
prescribTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
prescribTabDeath2.03 <- sqlQuery(sqldb,paste0("
...
labTabBirth2.03 <- sqlQuery(sqldb,paste0("
...
labTabDeath2.03 <- sqlQuery(sqldb,paste0("
...
DeathBeforeBirth <- sqlQuery(sqldb,paste0("
...
patsInEncTab <- sqlQuery(sqldb, paste0("select count(distinct person_id) as n from ",encounters))
...
for(i in 1:nrow(check3.03)){
  ...
  res3.03[[i]] <- with(check3.03, {sqlQuery(sqldb, 
...
patsWithEncs <- sqlQuery(sqldb, paste0("select count(*) as nrows, sum(patsWEncDiag) as patsWEncDiag, sum(patsWEncProc) as patsWEncProc from ( select  case when person_id in (select person_id from ", diagnoses, ") then 1 else 0 end as patsWEncDiag",
