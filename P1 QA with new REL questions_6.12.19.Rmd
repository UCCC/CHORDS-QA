...
knitr::opts_chunk$set(echo = TRUE,
                ...
                warning = FALSE)
startTime <- Sys.time()
tableReplaces <-  paste0(outputdir, "\\tablereplace.csv")
if (file.exists(tableReplaces)){
  print("updating for QA using tablereplace.csv")
  df <- read.csv2(tableReplaces, header = TRUE, sep = ",", stringsAsFactors=FALSE)
  
  demographics <- ifelse("demographics" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("demographics"), tolower(df$ORG_NAME))], "demographics")
  encounters <- ifelse("encounters" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("encounters"), tolower(df$ORG_NAME))], "encounters")
  census_location <- ifelse("census_location" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("census_location"), tolower(df$ORG_NAME))], "census_location")
  diagnoses <- ifelse("diagnoses" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("diagnoses"), tolower(df$ORG_NAME))], "diagnoses")
  vital_signs <- ifelse("vital_signs" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("vital_signs"), tolower(df$ORG_NAME))], "vital_signs")
  lab_results <- ifelse("lab_results" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("lab_results"), tolower(df$ORG_NAME))], "lab_results")
  procedures <- ifelse("procedures" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("procedures"), tolower(df$ORG_NAME))], "procedures")
  benefit <- ifelse("benefit" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("benefit"), tolower(df$ORG_NAME))], "benefit")
  linkage <- ifelse("linkage" %in% tolower(df$ORG_NAME),  df$NEW_NAME[match(tolower("linkage"), tolower(df$ORG_NAME))], "linkage")
...
con <- DBI::dbConnect(odbc::odbc(),
                      Driver = "SQL Server",
                      Server = "DWPHSQL01",
                      Database = "VDW_Aligned")
# connection to sql server
sqldb <- odbcDriverConnect(paste('driver={SQL Server};server=',"DWPHSQL01",';database=',"VDW_Aligned", sep=""), readOnlyOptimize = TRUE)
...
tab2_demo_null <- odbc::dbGetQuery(con, 
  paste0("SELECT SUM(CASE WHEN BIRTH_DATE IS NULL THEN 1 ELSE 0 END) as BIRTH_DATE,
   ...
   SUM(CASE WHEN RACE1 IS NULL THEN 1 ELSE 0 END) as RACE2,
   SUM(CASE WHEN RACE1 IS NULL THEN 1 ELSE 0 END) as RACE3,
   SUM(CASE WHEN RACE1 IS NULL THEN 1 ELSE 0 END) as RACE4,
   SUM(CASE WHEN RACE1 IS NULL THEN 1 ELSE 0 END) as RACE5,
...
tab2_demo_uk <- odbc::dbGetQuery(con, 
  paste0("SELECT SUM(CASE WHEN GENDER = 'U' THEN 1 ELSE 0 END) as GENDER,
   ...
   SUM(CASE WHEN RACE1 = 'UN' THEN 1 ELSE 0 END) as RACE2,
   SUM(CASE WHEN RACE1 = 'UN' THEN 1 ELSE 0 END) as RACE3,
   SUM(CASE WHEN RACE1 = 'UN' THEN 1 ELSE 0 END) as RACE4,
   SUM(CASE WHEN RACE1 = 'UN' THEN 1 ELSE 0 END) as RACE5,
...
demogCounts <- sqlQuery(sqldb, 
                  paste('SELECT gender, primary_language, race1, race2, race3, race4, race5, count(*) as nRows',
                        'FROM ', demographics,
                        'group by gender, primary_language, race1, race2, race3, race4, race5',
                        sep=" "
                        ),
...
## Table 4.1. Missing and unknown language count 

```{r other/missing primary language, echo=F}
...
## Table 6.6. Race1 by Race2 bivariate disribution
```{r race1xrace2 , echo=F}

demogCounts2 <- sqlQuery(sqldb, 
                  paste('SELECT race1, race2, race3, race4, race5, Hispanic, primary_language
                        FROM demographics'))
                  
race1xrace2 <- table(demogCounts2$race1,demogCounts2$race2)
rsum <- rowSums(race1xrace2)
race1xrace2 <- cbind(race1xrace2, rsum)
colnames(race1xrace2) <-c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')
colsum <-colSums(race1xrace2)
race1xrace2 <- rbind(race1xrace2, colsum)
rownames(race1xrace2) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')


knitr::kable(race1xrace2,
             format.args = list(big.mark = ","))
...
## Table 6.7. Race1 by Race3 bivariate disribution
```{r race1xrace3 , echo=F}

race1xrace3 <- table(demogCounts2$race1,demogCounts2$race3)
rsum <- rowSums(race1xrace3)
race1xrace3 <- cbind(race1xrace3, rsum)
colnames(race1xrace3) <-c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')
colsum <-colSums(race1xrace3)
race1xrace3 <- rbind(race1xrace3, colsum)
rownames(race1xrace3) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')


knitr::kable(race1xrace3,
             format.args = list(big.mark = ","))
...
## Table 6.8. Race1 by Race4 bivariate disribution
```{r race1xrace4 , echo=F}

race1xrace4 <- table(demogCounts2$race1,demogCounts2$race4)
rsum <- rowSums(race1xrace4)
race1xrace4 <- cbind(race1xrace4, rsum)
colnames(race1xrace4) <-c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')
colsum <-colSums(race1xrace4)
race1xrace4 <- rbind(race1xrace4, colsum)
rownames(race1xrace4) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')


knitr::kable(race1xrace4,
             format.args = list(big.mark = ","))

```
## Table 6.9. Race1 by Race5 bivariate disribution
```{r race1xrace5 , echo=F}

race1xrace5 <- table(demogCounts2$race1,demogCounts2$race5)
rsum <- rowSums(race1xrace5)
race1xrace5 <- cbind(race1xrace5, rsum)
race1xrace5
colnames(race1xrace5) <-c('Native Hawaiian or Other Pacific Islander', 'Unknown or Not Reported', 'White', 'Total')
colsum <-colSums(race1xrace5)
race1xrace5 <- rbind(race1xrace5, colsum)
rownames(race1xrace5) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')


knitr::kable(race1xrace5,
             format.args = list(big.mark = ","))
...
```{r race1xethnicity , echo=F}
race1xethnicity <- table(demogCounts2$race1,demogCounts2$Hispanic)
rsum <- rowSums(race1xethnicity)
race1xethnicity  <- cbind(race1xethnicity, rsum)
colnames(race1xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race1xethnicity)
race1xethnicity <- rbind(race1xethnicity, colsum)

rownames(race1xethnicity) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')

knitr::kable(race1xethnicity,
             format.args = list(big.mark = ","))
```             
## Table 7.2 Race2 by Hispanic Ethnicity where Race1 is unknown
```{r race2xethnicity , echo=F}

demogCounts3 <- subset(demogCounts2, race1=='UN')
race2xethnicity <-table(demogCounts3$race2, demogCounts3$Hispanic)
rsum <- rowSums(race2xethnicity)
race2xethnicity  <- cbind(race2xethnicity, rsum)
colnames(race2xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race2xethnicity)
race2xethnicity <- rbind(race2xethnicity, colsum)
rownames(race2xethnicity) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')

knitr::kable(race2xethnicity,
             format.args = list(big.mark = ","))
```        
## Table 7.3 Race3 by Hispanic Ethnicity where Race1 is unknown
```{r race3xethnicity , echo=F}

race3xethnicity <-table(demogCounts3$race3, demogCounts3$Hispanic)
rsum <- rowSums(race3xethnicity)
race3xethnicity  <- cbind(race3xethnicity, rsum)
colnames(race3xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race3xethnicity)
race3xethnicity <- rbind(race3xethnicity, colsum)
rownames(race3xethnicity) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')

knitr::kable(race3xethnicity,
             format.args = list(big.mark = ","))
```        


## Table 7.4 Race4 by Hispanic Ethnicity where Race1 is unknown
```{r race4xethnicity , echo=F}

race4xethnicity <-table(demogCounts3$race4, demogCounts3$Hispanic)
rsum <- rowSums(race4xethnicity)
race4xethnicity  <- cbind(race4xethnicity, rsum)
colnames(race4xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race4xethnicity)
race4xethnicity <- rbind(race4xethnicity, colsum)
rownames(race4xethnicity) <- c('Asian', 'Black or African American', 'Native Hawaiian or Other Pacific Islander',
                                 'American Indian/Alaska Native', 'Unknown or Not Reported', 'White', 'Total')

knitr::kable(race4xethnicity,
             format.args = list(big.mark = ","))
```        
## Table 7.5 Race5 by Hispanic Ethnicity where Race1 is unknown
```{r race5xethnicity , echo=F}

race5xethnicity <-table(demogCounts3$race5, demogCounts3$Hispanic)
rsum <- rowSums(race5xethnicity)
race5xethnicity  <- cbind(race5xethnicity, rsum)
colnames(race5xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race5xethnicity)
race5xethnicity <- rbind(race5xethnicity, colsum)
rownames(race5xethnicity) <- c('Native Hawaiian or Other Pacific Islander',
                                 'Unknown or Not Reported', 'White', 'Total')

knitr::kable(race5xethnicity,
             format.args = list(big.mark = ","))
```
## Table 7.6 Race1 by Hispanic Ethnicity where primary language is Spanish
```{r race1xethnicityxlang , echo=F}
demogCounts4 <- subset(demogCounts2, primary_language=='spa')
race1xethnicity <-table(demogCounts4$race1, demogCounts4$Hispanic)
rsum <- rowSums(race1xethnicity)
race1xethnicity  <- cbind(race1xethnicity, rsum)
colnames(race1xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race1xethnicity)
...
```{r race2xethnicityxlang , echo=F}
race2xethnicity <-table(demogCounts4$race2, demogCounts4$Hispanic)
rsum <- rowSums(race2xethnicity)
race2xethnicity  <- cbind(race2xethnicity, rsum)
colnames(race2xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race2xethnicity)
...
```{r race3xethnicityxlang , echo=F}
race3xethnicity <-table(demogCounts4$race3, demogCounts4$Hispanic)
rsum <- rowSums(race3xethnicity)
race3xethnicity  <- cbind(race3xethnicity, rsum)
colnames(race3xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race3xethnicity)
...
```{r race4xethnicityxlang , echo=F}
race4xethnicity <-table(demogCounts4$race4, demogCounts4$Hispanic)
rsum <- rowSums(race4xethnicity)
race4xethnicity  <- cbind(race4xethnicity, rsum)
colnames(race4xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race4xethnicity)
...
```{r race5xethnicityxlang , echo=F}
race5xethnicity <-table(demogCounts4$race5, demogCounts4$Hispanic)
rsum <- rowSums(race5xethnicity)
race5xethnicity  <- cbind(race5xethnicity, rsum)
colnames(race5xethnicity) <-c('Not Hispanic', 'Unknown Hispanic', 'Hispanic','Total')
colsum <-colSums(race5xethnicity)
...
enc <- sqlQuery(sqldb,'select a.person_id, a.adate, b.person_id, b.race1, b.Hispanic
from encounters as a
left join demographics as b
on a.person_id=b.person_id
where b.race1==UN and b.Hispanic==U'
                  )
enc

demogCounts2 <- sqlQuery(sqldb, 
                  paste('SELECT person_id, ADATE FROM encounters as a
                  left join demographics, person_id, race1, Hispanic,
                        FROM demographics'))
...
```{r Count of diagnoses by year and month in ambulatory encounters, echo=F}
...
```{r Count of diagnoses by year and month other encounter types, echo=F}
...
```{r vital_signs table Table, echo=F}
...
```{r trends in the number of recorded vital signs, tab16_res, echo = FALSE}
...
```{r Count of vital signs by year and month, echo=F}
...
```{r tab17_res, eval = F}
tab17_ben_null <- odbc::dbGetQuery(con, 
  paste0("SELECT COUNT(*) as total,
   SUM(CASE WHEN LOAD_DATE IS NULL THEN 1 ELSE 0 END) as LOAD_DATE,
   MIN(LOAD_DATE) as MIN_LOAD_DATE,
   MAX(LOAD_DATE) as MAX_LOAD_DATE,
   SUM(CASE WHEN REFRESH_DATE IS NULL THEN 1 ELSE 0 END) as REFRESH_DATE,
   ...
   SUM(CASE WHEN END_DATE IS NULL THEN 1 ELSE 0 END) as END_DATE,
   MIN(END_DATE) as MIN_END_DATE,
   MAX(END_DATE) as MAX_END_DATE
      FROM ", benefit))
tab17_final <- tab17_ben_null %>% 
  mutate(
    perc_null_load = 100* (LOAD_DATE / total),
    perc_null_refresh = 100* (REFRESH_DATE / total),
    perc_null_start = 100* (START_DATE / total),
    perc_null_end = 100* (END_DATE / total)
  )
tab17_display  <- list(
  "LOAD_DATE" =
    list("Minimum" = ~ min(MIN_LOAD_DATE, na.rm = T),
         "Maximum" = ~ max(MAX_LOAD_DATE, na.rm = T),
         "Percent Null" = ~ paste0(perc_null_load, "%")),
  "REFRESH_DATE" =
    list("Minimum" = ~ min(MIN_REFRESH_DATE, na.rm = T),
         "Maximum" = ~ max(MAX_REFRESH_DATE, na.rm = T),
         "Percent Null" = ~ paste0(perc_null_refresh, "%")),
  "END_DATE" =
    list("Minimum" = ~ min(MIN_START_DATE, na.rm = T),
         "Maximum" = ~ max(MAX_START_DATE, na.rm = T),
         "Percent Null" = ~ paste0(perc_null_start, "%")),
  "END_DATE" =
    list("Minimum" = ~ min(MIN_END_DATE, na.rm = T),
         "Maximum" = ~ max(MAX_END_DATE, na.rm = T),
         "Percent Null" = ~ paste0(perc_null_end, "%")),
  "BENEFIT_TYPE" =
    list("Current insurance or benefit information" = ~ qwraps2::perc_n(BENEFIT_TYPE == "CU", na_rm = T),
         "Enrollment insurance or benefit information" = ~ qwraps2::perc_n(BENEFIT_TYPE == "EN", na_rm = T),
         "Primary payer on a claim or encounter" = ~ qwraps2::perc_n(BENEFIT_TYPE == "PR", na_rm = T),
         "Payer on a claim with unknown rank or order" = ~ qwraps2::perc_n(BENEFIT_TYPE == "PA", na_rm = T),
         "Self-reported insurance or benefit information" = ~ qwraps2::perc_n(BENEFIT_TYPE == "SR", na_rm = T),
         "No Benefit information available" = ~ qwraps2::perc_n(BENEFIT_TYPE == "NI", na_rm = T),
         "NULL" = ~ qwraps2::perc_n(BENEFIT_TYPE == "NULL", na_rm = T),
         "Other/None of the above" = ~ qwraps2::perc_n(!(BENEFIT_TYPE %in% c("CU", "EN", "PR", "PA", "SR", "NI", "NULL")), na_rm = T)),
  "BENEFIT_CAT" =
    list("Correctional Care" = ~ qwraps2::perc_n(BENEFIT_TYPE == "CC", na_rm = T),
         "Commercial/Private Insurance" = ~ qwraps2::perc_n(BENEFIT_TYPE == "CO", na_rm = T),
         "Children’s Health Insurance Program " = ~ qwraps2::perc_n(BENEFIT_TYPE == "CP", na_rm = T),
         "Medicare" = ~ qwraps2::perc_n(BENEFIT_TYPE == "MC", na_rm = T),
         "Medicaid" = ~ qwraps2::perc_n(BENEFIT_TYPE == "MD", na_rm = T),
         "Other government" = ~ qwraps2::perc_n(BENEFIT_TYPE == "OG", na_rm = T),
         "Self-Pay/Uninsured/Private Pay " = ~ qwraps2::perc_n(BENEFIT_TYPE == "SP", na_rm = T),
         "Other insurance" = ~ qwraps2::perc_n(BENEFIT_TYPE == "OT", na_rm = T),
         "NULL" = ~ qwraps2::perc_n(BENEFIT_TYPE == "NULL", na_rm = T),
         "Other/None of the above" = ~ qwraps2::perc_n(!(BENEFIT_TYPE %in% c("CC", "CO", "CP", "MC", "MD", "OG", "SP", "OT", "NULL")), na_rm = T))
) 
```

```{tab17_disp, results = "asis", eval=F}
tab17_final <- qwraps2::summary_table(tab17_final, tab17_display, digits = 2)
```

## Table 18: CENSUS_LOCATION

```{r tab18_out, results="asis", echo=F}
tab18a <- odbc::dbGetQuery(con, 
                           paste0("SELECT GEOCODE_COUNTY,
                                  COUNT(*) as num_county
                                  FROM ", census_location, "
                                  GROUP BY GEOCODE_COUNTY"))
tab18b <- odbc::dbGetQuery(con, 
                           paste0("SELECT ADDRESS_TYPE_CODE,
                                  COUNT(*) as num_add_flag
                                  FROM ", census_location, "
                                  GROUP BY ADDRESS_TYPE_CODE"))
tab18_tot <- odbc::dbGetQuery(con, 
                              paste0("SELECT COUNT(*) n_census
                                     FROM ", census_location))
tab18_allCounty <- tab18a %>%
  mutate(
    perc_county = paste0(round((num_county / tab18_tot$n_census) * 100, 1), "%")
  )
tab18_allAddType <- tab18b %>%
  mutate(
    perc_add_type_code = paste0(round((num_add_flag / tab18_tot$n_census) * 100, 1), "%")
  )
knitr::kable(tab18_allCounty, col.names = c("County Geocode", "Frequency County Geocode", "Percent County Geocode"), format.args = list(big.mark = ","))
knitr::kable(tab18_allAddType, col.names = c("Address Flag Type Code", "Frequency Address Type Code", "Percent Address Type Code"), format.args = list(big.mark = ","))
```

## Table 19: LINKAGE table

```{r, echo=FALSE, echo = F}
tab19 <- odbc::dbGetQuery(con, 
...
knitr::kable(tab19, col.names = c("Number of unique patient IDs", "Number of unique linkage IDs"), format.args = list(big.mark = ","))
```
...
title: "P1 QA report with new REL questions"
author: "Emily Bacon"
date: "6/4/2019"
